    ...
    public class LocalizationService : ObservableObject
    {
        ...
        private static Dictionary<string, string> BuildEnglish()
        {
            return new Dictionary<string, string>
            {
                ...
                ["Dashboard.Status.VerifiedViaHook"] = "Verified via Hook",
                ["Dashboard.Status.HookLoadedNoMenu"] = "Hook Loaded (No Menu)",
                ["Dashboard.Status.OrphanedMissingDll"] = "Orphaned / Missing DLL",
                ["Dashboard.Status.IpcTimeout"] = "IPC Timeout",
                ["Dashboard.Status.LoadError"] = "Load Error",
                ["Dashboard.Status.RegistryFallback"] = "Registry Fallback",
                ["Dashboard.Status.StaticNotMeasured"] = "Static (Not Measured)",
                ["Dashboard.Status.SkippedKnownUnstable"] = "Skipped (Known Unstable)",
        ...
        private static Dictionary<string, string> BuildChinese()
        {
            return new Dictionary<string, string>
            {
                ...
                ["Dashboard.Status.VerifiedViaHook"] = "已由 Hook 验证",
                ["Dashboard.Status.HookLoadedNoMenu"] = "Hook 已加载（无菜单）",
                ["Dashboard.Status.OrphanedMissingDll"] = "孤立 / 缺失 DLL",
                ["Dashboard.Status.IpcTimeout"] = "IPC 超时",
                ["Dashboard.Status.LoadError"] = "加载错误",
                ["Dashboard.Status.RegistryFallback"] = "注册表回退",
                ["Dashboard.Status.StaticNotMeasured"] = "静态（未测量）",
                ["Dashboard.Status.SkippedKnownUnstable"] = "已跳过（已知不稳定）",
